#!/usr/bin/env bash
# Push a Liquid selftest command to an iOS simulator and poll for the result.
# Usage:
#   send-ios-simulator-liquid-command.sh sync [wallet_id]
#   send-ios-simulator-liquid-command.sh transfer <destination_address> <amount_sats> [wallet_id]
set -euo pipefail

LOG_ROOT="${REDWALLET_LOG_ROOT:-/Volumes/T705/redwallet-logs}"
STAMP="$(date +%Y%m%d-%H%M%S)"
RUN_DIR="${REDWALLET_IOS_SIM_LIQUID_COMMAND_LOG_DIR:-$LOG_ROOT/ios-sim-liquid-command-$STAMP}"
...
POLL_SECONDS="${REDWALLET_IOS_LIQUID_COMMAND_POLL_SECONDS:-180}"
RPC_URL="${LIQUID_RPC_URL:-http://127.0.0.1:18443/wallet/redwallet-proof}"
ELECTRUM_URL="${LIQUID_ELECTRUM_URL:-tcp://127.0.0.1:60401}"
SIM_APP_LAUNCHED=0
DESTINATION=""
AMOUNT=""
WALLET_ID="${REDWALLET_IOS_LIQUID_WALLET_ID:-}"

mkdir -p "$RUN_DIR"
ln -sfn "$RUN_DIR" "${LOG_ROOT%/}/current-ios-sim-liquid-command"
...
command_id="ios-sim-liquid-${OPERATION}-${STAMP}"
...
if [[ "$matched_result" != 1 ]]; then
  log "BLOCKER no matching Liquid result for operation=$OPERATION command_id=$command_id"
...
echo "export IOS_LIQUID_${op_upper}_TXID='$txid'" >"$RUN_DIR/ios-liquid-${OPERATION}.env"
cat "$RUN_DIR/ios-liquid-${OPERATION}.env"
